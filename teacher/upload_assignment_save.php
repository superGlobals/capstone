...
if ($errflag) {
   ...
   <script>
   window.location = 'uploaded_assignment.php<?php echo '?id='.$get_id;  ?>';
   </script>
   <?php exit();
}
...
if ((!empty($_FILES["uploaded_file"])) && ($_FILES['uploaded_file']['error'] == 0)) {
    ...
    if (($ext != "exe") && ($_FILES["uploaded_file"]["type"] != "application/x-msdownload")) {
		...
		$name_notification  = 'Upload Assignment file name'." ".'<b>'.$name.'</b>';
        ...
        if (!file_exists($newname)) {
            ...
            if ((move_uploaded_file($_FILES['uploaded_file']['tmp_name'], $newname))) {
				...
				mysqli_query($conn,"INSERT INTO `assignment`(`floc`, `fdatein`, `fdesc`, `teacher_id`, `class_id`, `fname`, `uploaded_by`) VALUES ('$newname',NOW(),'$filedesc','$session_id','$id[$i]','$name','$uploaded_by')");


				mysqli_query($conn,"insert into notification (teacher_class_id,notification,date_of_notification,link) value('$id[$i]','$name_notification',NOW(),'assignment_files.php')")or die(mysqli_error());
